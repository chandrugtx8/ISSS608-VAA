...
12.4 Data Import and Prepatation 12.4.1 Importing geospatial data into R environment In this section, you will import Myanmar Township Boundary GIS data and its associated attrbiute table into R environment.
...
```{r}
shan_sf <- st_read(dsn = "../Hands_on_Ex12/data/geospatial", 
...
12.4.2 Importing aspatial data into R environment The csv file will be import using read_csv function of readr package.

The code chunks used are shown below:

```{r}
ict <- read_csv ("../Hands_on_Ex12/data/aspatial/Shan-ICT.csv")
```
...
write_rds(shan_sf, "../Hands_on_Ex12/data/rds/shan_sf.rds")
```
...
```{r}
shan_sf <- read_rds("../Hands_on_Ex12/data/rds/shan_sf.rds")
```
...
Next, we need to change the rows by township name instead of row number by using the code chunk

shan_ict \<- read_rds("../Hands_on_Ex12/data/rds/shan_ict.rds")nk below
...
```{r}
# --- Build clustering table ---------------------------------------------------
cluster_vars <- shan_sf %>%
  st_set_geometry(NULL) %>%
  dplyr::select("TS.x", "RADIO_PR", "TV_PR", "LLPHONE_PR", "MPHONE_PR", "COMPUTER_PR")

# Use township name as row names
row.names(cluster_vars) <- cluster_vars$TS.x

# Keep only the 5 numeric variables
shan_ict <- dplyr::select(cluster_vars, 2:6)

# Quick peek
head(shan_ict, 10)

# --- Persist for later chunks (optional) --------------------------------------
dir.create("data/rds", recursive = TRUE, showWarnings = FALSE)
readr::write_rds(shan_ict, "data/rds/shan_ict.rds")
```

```{r}
shan_ict <- read_rds("../Hands_on_Ex12/data/rds/shan_ict.rds")
```
...
12.7.11.2 Plotting interactive cluster heatmap using heatmaply() In the code chunk below, the heatmaply() of heatmaply package is used to build an interactive cluster heatmap.
...
12.9.2.1 Mapping the clusters formed Similarly, we can plot the clusters on a categorical area shaded map by using the steps we learned in 5.7.12 Mapping the clusters formed.
