...
```{r}
kbb <- st_read(dsn="../Hands_on_Ex06/data/rawdata",
...
```{r}
kbb_sf <- st_read(dsn="../Hands_on_Ex06/data/rawdata",
               layer = "Kepulauan_Bangka_Belitung") %>%
  st_union() %>%
...
### 6.5.2 Converting OWIN Next, as.owin() is used to convert kbb into an owin object.
...
```{r}
fire_sf <- read_csv("../Hands_on_Ex06/data/rawdata/forestfires.csv") %>%
...
### 6.9.1 Creating ppp object In the code chunk below, DayofYear field is included in the output ppp object.
