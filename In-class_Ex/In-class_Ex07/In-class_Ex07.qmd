...
Best bandwidth = 27 nearest neighbours This is because AICc = 41978.63 is the lowest among all computed values the model with this bandwidth achieves the best trade-off between fit and complexity.
...
```{r}
# 1) Find the Planning Area column automatically
pa_col <- names(mpsz_svy21)[grepl("PLN|PLAN.*AREA", names(mpsz_svy21), ignore.case = TRUE)][1]

# 2) Filter for Yishun instead of Tampines
mpsz_yishun <- mpsz_svy21 %>% filter(!!rlang::sym(pa_col) == "YISHUN")

# 3) Keep only points that fall inside Yishun
pts_yishun <- st_join(condo_resale.sf.adaptive, mpsz_yishun["SUBZONE_N"], join = st_within, left = FALSE)
# (left = FALSE drops points outside; use left=TRUE if you want to keep NAs)

# 4) Plot Local R² for Yishun only
tm_shape(mpsz_yishun) +
  tm_polygons(col = NA) +               # transparent polygons
  tm_borders(col = "gray60", lwd = 1) + # border for context
tm_shape(pts_yishun) +
  tm_dots(col = "Local_R2",
          palette = "-RdYlBu",          # diverging palette
          style = "fixed",              # consistent color breaks
          breaks = seq(0.88, 0.98, 0.02),  # 0.88–0.98 in steps of 0.02
          size = 0.35,
          border.col = "gray20",
          border.lwd = 0.4,
          title = "Local R² (Yishun)") +   # updated title
