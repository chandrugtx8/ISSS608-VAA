...
# Safety: if you didn’t keep lw_q from §5, rebuild here
if (!exists("lw_q")) {
  nb_q <- sfdep::st_contiguity(hex)                    # queen contiguity neighbours
  lw_q <- sfdep::st_weights(nb_q, allow_zero = TRUE)   # keep islands
}

# 6.1 Window helpers (same logic as §4)
is_wk_am <- function(x_day, x_hr)  x_day == "WEEKDAY"          & x_hr %in% 6:9
is_wk_pm <- function(x_day, x_hr)  x_day == "WEEKDAY"          & x_hr %in% 17:20
is_wknd  <- function(x_day, x_hr)  x_day == "WEEKENDS/HOLIDAY" & x_hr %in% 11:20

# 6.2 Build monthly-wide table for any window
agg_monthly <- function(od_pts_sf, hex_sf, is_window_fn) {
  # aggregate trips by hex_id x year_month and widen
  trips_wide <- od_pts_sf %>%
    filter(is_window_fn(day_type, time_per_hour)) %>%
    st_drop_geometry() %>%
    count(hex_id, year_month, wt = total_trips, name = "trips") %>%
    pivot_wider(names_from = year_month, values_from = trips, values_fill = 0)

  # make a plain tibble of all hex IDs to guarantee a 1:1 canvas
  hex_index <- hex_sf %>% st_drop_geometry() %>% as_tibble() %>% select(hex_id)

  # left join so hexes with no trips still appear with zeros
  wide_on_index <- hex_index %>%
    left_join(trips_wide, by = "hex_id") %>%
    arrange(hex_id)

  # return as sf by joining geometry back
  hex_sf %>% left_join(wide_on_index, by = "hex_id")
}

# 6.3 Compute monthly Gi* time series (localG returns z-scores)
localGi_over_time <- function(sf_wide, lw, month_cols) {
  gi_mat <- map_dfc(month_cols, function(m) {
    x <- sf_wide[[m]]
    # guard against NAs that would stop spdep::localG
    x[is.na(x)] <- 0
    z <- spdep::localG(x, lw)           # vector of z-scores
    tibble(!!paste0("gi_", m) := as.numeric(z))
  })
  bind_cols(sf_wide %>% select(hex_id), gi_mat) %>% 
    st_as_sf(sf_wide %>% st_geometry())
}

# 6.4 A very light EHSA labeller:
#     MK trend on |Gi*| (strength) + a “new hot/cold” for last-month z>=1.96
label_each_hex <- function(gi_sf, month_cols) {
  gi_cols <- paste0("gi_", month_cols)
  last_col <- gi_cols[length(gi_cols)]

  # MK trend on absolute Gi* (strength increases or decreases)
  mk <- apply(st_drop_geometry(gi_sf)[, gi_cols, drop = FALSE], 1, function(v) {
    v[is.na(v)] <- 0
  ...
  # “new hot/cold” if last month significant (|z|≥1.96) and previous months were weak
  prev_max <- apply(st_drop_geometry(gi_sf)[, gi_cols[-length(gi_cols)], drop = FALSE],
...
# 6.5 Build monthly wide tables for AM/PM/WE
am_wide <- agg_monthly(od_pts, hex, is_wk_am)
pm_wide <- agg_monthly(od_pts, hex, is_wk_pm)
we_wide <- agg_monthly(od_pts, hex, is_wknd)

# identify the month columns (everything except id/geometry)
...
# quick sanity – how many months per window?
sapply(list(AM = am_month_cols, PM = pm_month_cols, WE = we_month_cols), length)

# 6.6 Gi* time series & EHSA-style labels
...
# 6.7 Small QC tables (should not be all “Insufficient” anymore)
table_AM <- sort(table(ehsa_am$ehsa), decreasing = TRUE)
table_PM <- sort(table(ehsa_pm$ehsa), decreasing = TRUE)
table_WE <- sort(table(ehsa_we$ehsa), decreasing = TRUE)
table_AM; table_PM; table_WE

# 6.8 Plot (static) – set component.autoscale = FALSE to prevent legend shrink
tmap_mode("plot"); tmap_options(component.autoscale = FALSE)

tm1 <- tm_shape(ehsa_am) + tm_polygons("ehsa", title = "EHSA (AM)")
tm2 <- tm_shape(ehsa_pm) + tm_polygons("ehsa", title = "EHSA (PM)")
tm3 <- tm_shape(ehsa_we) + tm_polygons("ehsa", title = "EHSA (WE)")
tm1
tm2
tm3
